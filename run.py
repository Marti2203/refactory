...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-q",
        "--questions",
        help="a sequence of question names.",
        nargs="+",
        default=None,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-s",
        "--sampling_rates",
        help="a sequence of sampling rates.",
        nargs="+",
        type=int,
    ...
    parser.add_argument(
        "-e",
        "--exp_num",
        help="the number of experiemnts for each sampling rate.",
        nargs="?",
        type=int,
        default=1,
    )
    ...
    parser.add_argument(
        "-c",
        "--cmb_log",
        help="combine log files into one.",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "-y",
        "--oro_json",
        help="only do only refactoring and store the results.",
        action="store_true",
        default=False,
    )
    ...
    if not (
        args.offline_refactoring
        or args.online_refactoring
        or args.block_repair
        or args.cmb_log
        or args.online_refactoring_only
    ):
        ...
        for sr in sr_list:
            if sr == 0 or sr == 100:  # No repetitions since no real sampling
                ...
                for exp_idx in range(
                    ...
                    ofl_refactor(
                        args.data_dir, args.questions, sampling_rate=sr, exp_idx=exp_idx
                    )
