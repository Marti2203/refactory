...
class Match:
    ...
    def reset(self):
        ...
        # Overall success;
        self.success = (
            None  # None=>Unknown, True=>Matching complete, False=>Matching failed
        )
        ...
        self.jumpFlag = (
            None  # None=>Unknown, True=>Waiting for jump, False=>Matching complete
        )
        # Waiting for return instruction? (when encounter RETURN-tag). Match success incomplete if this is True
        self.returnFlag = (
            None  # None=>Unknown, True=>Waiting for return, False=>Matching complete
        )
        # Waiting for Break instruction? (when encounter BREAK-tag). Match success incomplete if this is True
        self.breakFlag = (
            None  # None=>Unknown, True=>Waiting for break, False=>Matching complete
        )
...
class Matcher:
    ...
    def setReturn(self, nodeO):
        ...
        if type(nodeO) == ast.Return:  # Check if return instruction found
            ...
            prevMatch.returnFlag is None
        ):  # Else if return instruction not found earlier
    ...
    def add(self, tempSuccess, recLevel, delNodes):
        ...
        if (
            not prevMatch.jumpFlag
            and not prevMatch.returnFlag
            and not prevMatch.breakFlag
            and tempSuccess
            and recLevel == 1
        ):
    ...
    if type(nodeR) is list:
        if type(nodeO) is list:
            ...
            return (
                nodeO.body,
                nodeR,
            )  # Orig list has at least as many nodes as Rule list
...
def matchRule_name(nodeO, nodeR, matcher, recLevel=1, verbose=False):
    ...
    if name:
        ...
        if "BREAK" in name:  # If waiting for BREAK match
            appendToBlock = matcher.setBreak(
                nodeO
            )  # Don't append successful break matches
...
def matchRule_list(nodeO, nodeR, matcher, recLevel=1, verbose=False):
    ...
    if childrenO is not None and childrenR is not None:
        ...
        success = (
            len(nodeR) == 0 and len(nodeO) == 0
        )  # If empty lists, nothing to match. Else false
        numCombinations = (
            len(childrenO) - len(childrenR) + 1
        )  # Try as many as oCr combinations
        ...
        for initI in range(
            numCombinations
        ):  # Try all consecutive combinations of Orig list
            indexR, indexO, tempSuccess, delNodes = 0, 0, False, []

            while indexR < len(childrenR) and initI + indexO < len(
                childrenO
            ):  # For each orig-rule elementwise pair
                childO, childR = childrenO[initI + indexO], childrenR[indexR]
                tempSuccess = matchRule(
                    childO, childR, matcher, recLevel=recLevel + 1, verbose=verbose
                )
                ...
                if not tempSuccess:  # failed childO-childR match
                    matcher.reset(
                        recLevel
                    )  # reset and try next zip (consecutive combination)
                    break
                ...
                if (
                    not matcher.getBlock()
                ):  # If greedy switch on, don't increment indexR
                    indexR += 1
...
def matchRule_type(nodeO, nodeR, matcher, recLevel=1, verbose=False):
    ...
    if type(nodeR) is not list and type(nodeR) == type(
        nodeO
    ):  # If not list, and same type
        ...
        if hasattr(nodeO, "_fields"):  # Match all fields
            for (
                ...
                nodeO._fields
            ):  # Field names are same, since nodeO and nodeR are same type
                pprint(
                    verbose,
                    recLevel + 1,
                    "field=" + field,
...
def matchOrig(tree, rule, nodeO=None, nodeR=None, verbose=False):
    ...
    for match in matcher.matches:
        if match.success:
            treeCopy, nodeCopy = ruleAction.applyAction(
                tree, rule, nodeO, match.replHash, match.delNodes, verbose=verbose
            )
    ...
    if hasattr(nodeO, "body"):  # If there exists a 'body'
        for i in nodeO.body:  # For each childNode inside the body
            tree, nodeO, newReplNodes = matchOrig(
                tree, rule, i, nodeR, verbose=verbose
            )  # match ruleP against child
