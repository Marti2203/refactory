...
def main_controlFlow():
    ...
    buggyCFs, correctCFs = controlFlow.get_buggyCorr_cfs(
        buggyCodes, correctCodes, cf_single, cf_mult
    )
...
class Refactor:
    def __init__(self, fname, origCode, depth=1):
        ...
        self.depth = (
            depth  # How many far away (#rule-applications) is it from code in fname?
        )
    ...
    def __str__(self):
        ...
        for ruleName in self.ruleAppls:
            stri += (
                ...
                + "-" * 3
                + ": #"
                + str(len(self.ruleAppls[ruleName]))
                + "\n"
            )
...
def applyRules(refactor: Refactor, untilDepth, results, currDepth=1):
    ...
    for r in rule.rules:
        corrParse = ast.parse(
            refactor.origCode
        )  # Run parse for each rule, since original parse modified!
        verbose = (r.name in verboseRnames) and (refactor.fname in verboseFnames)
        if verbose:
            print(
                "-" * 50,
                "\nDebugging fname=" + refactor.fname,
                "rname=" + r.name,
                "\n" + "-" * 50,
            )
        else:
    ...
    if untilDepth != currDepth:  # Depth not exhausted
        applyRules_rec(
            refactor, untilDepth, results, currDepth=currDepth
        )  # Apply on all newly generated
        ...
        if refactor.depth == currDepth:  # that was newly generated in current run
            for rname in refactor.ruleAppls:  # For each rule
                for newCode in refactor.ruleAppls[rname]:  # For each new code
                    newRefactor = Refactor(
                        refactor.fname, newCode, depth=refactor.depth + 1
                    )  #
                    applyRules(
                        newRefactor,
