...
def prePrint(verbose, replHash, replHashCopy, delNodes, delNodesCopy, tree, treeCopy):
    if verbose:
        print("applyAction.ReplHash-Orig: ")
        print(replHash)
        print("applyAction.ReplHash-Copy: ")
        print(replHashCopy)

        print("\napplyAction.DelNode-Orig: ")
        print(delNodes)
        print("applyAction.DelNode-Copy: ")
        print(delNodesCopy)

        print("\napplyAction.Tree-Orig: ")
        printTree(tree)
        print("applyAction.Tree-Copy: ")
        printTree(treeCopy)
    ...
    # Otherwise, replace each field
        ...
        for field in nodeA._fields:
            ...
            if repl:
                ...
                setattr(nodeA, field, repl_placeholders(attr, replHash))
...
def recurseDel(nodeO, delNode, depth=0):
    ...
    if "body" in nodeO._fields:
        ...
        for child in nodeO.body:  # Otherwise
            i, depth = recurseDel(
                child, delNode, depth + 1
            )  # Search within its children
            if i is not None:
                return i, depth
...
def applyAction(tree, rule, nodeO, replHash, delNodes, verbose=False):
    ...
    for delNode in delNodesCopy:
        ...
        if depth == 0:  # Add index to min, only if when depth = 0
            lowestI = min(
                lowestI, index
            )  # i.e, delNode found at current tree level (nodeO.body)
